...
├── hashing/                            Password hashing (v0.8.2)
│   ├── driver/                         Hasher interface + Spec + registry
│   └── drivers/                        bcrypt · argon2id · scrypt (all light, auto-registered)
├── encryption/                         Symmetric encryption (v0.8.3)
│   ├── driver/                         Cipher interface + Spec + registry
│   ├── encrypter.go                    Encrypter + key ring + token format
│   └── drivers/                        aesgcm · chacha20poly1305 (light, auto-registered)
...
├── validation/                         Struct validator (v0.9.0)
│   ├── validator.go                    Validator + ValidateStruct / ValidateField
│   ├── parser.go                       Struct-tag DSL parser
│   ├── translator.go                   MapTranslator + English templates
│   └── rules_*.go                      Built-in rule implementations
├── httpclient/                         HTTP client (v0.9.1)
│   ├── driver/                         Public driver contract
│   ├── drivers/                        stdlib · mock · resilient (auto)
│   └── middleware/                     OTel RoundTripper
├── ratelimit/                          Token-bucket rate limiter (v0.9.2)
...
| 0.9.x | Validation + HTTP client + rate limit | **0.9.0: `validation`** · **0.9.1: `httpclient`** · **0.9.2: `ratelimit` (memory + redis token bucket + middleware)** |
