...
├── httpclient/ ratelimit/              Future (v0.9.x)
...
| 0.9.x | Validation + HTTP client + rate limit | **0.9.0: `validation` (struct-tag DSL + i18n + 30+ rules)** · 0.9.1: `httpclient` (stdlib + resilient + OTel) · 0.9.2: `ratelimit` (memory + redis token bucket + middleware) |
