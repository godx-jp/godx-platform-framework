...
|----------|------|---------|---------|
| `OBS_BACKEND` | enum | `stdout` | Driver selector: `stdout` · `otlp` · `cloudwatch` |
...
## Observability — CloudWatch driver (0.2.0+)
...
Standard AWS credential resolution (env, IRSA, EC2 metadata, `~/.aws/credentials`) applies once the 0.2.0 driver lands.
...
OBS_BACKEND=stdout       # stdout | otlp | cloudwatch
OBS_LOG_LEVEL=info
OBS_TRACE_SAMPLE=1.0
...
# Required when OBS_BACKEND=cloudwatch (0.2.0+)
