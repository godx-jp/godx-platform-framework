...
// FromApp returns the Manager published by mail.Module on app.
func FromApp(app *framework.App) (*Manager, error) {
	v, ok := app.Lookup(StoreKey)
	if !ok {
		return nil, fmt.Errorf("mail: Module has not been initialised on this App")
	}
	mgr, ok := v.(*Manager)
	if !ok {
		return nil, fmt.Errorf("mail: %s framework Store entry is not a *Manager (%T)", StoreKey, v)
	}
	return mgr, nil
}
